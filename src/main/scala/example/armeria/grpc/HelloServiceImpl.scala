...
import example.armeria.grpc.HelloServiceImpl.{blockingContextAwareExecutionContext, contextAwareScheduler, toMessage}
...
class HelloServiceImpl extends HelloServiceGrpc.HelloService {
  ...
  override def hello(request: HelloRequest): Future[HelloReply] = {
    println(s"ServiceReq Ctx ctx ${ServiceRequestContext.current().hashCode()}")
    println(s"Kamon ctx ${Kamon.currentContext().hashCode()}")
